...
进制运算（某一进制数的大小由系数项和权的乘积决定）
例子:
(123.45)10=1*10^2+2*10^1+3*10^0+4*10^-1+5*10^-2
(10101.11)2=1*2^4+1*2^2+1*2^0+1*2^-1+1*2^-2
(375.4)8=3*8^2+7*8^1+5*8^0+4*8^-1
...
机器数的正负：一个数的最高位表示符号0：为正，1为负
例如：11010111=-87

这样：连同符号一起码化的数被称之为机器数： 11010111
而，使用正负号加其绝对值的表示方法，称之为该数的真值  ：-87


原码：指将最高位作为符号位（0表示正，1表示负），其他数字位代表数值本身的绝对值的数字表示方式
	数字6在计算机中原码表示为 0000 0110
	数字-6在计算机中原码表示为 1000 0110
 
 原码运算：以上是在8位计算机中的原码表示，如果在32位或者16位计算机中，表示方法是一样的，只是多加几个0
	计算机中进行计算，减法都是转化成对应的加法来进行计算的，因为加法的效率比较高的（
	减法也需要转化为加法
	原码在进行乘除运算的时候没有问题，但是在进行加减的时候会有问题

反码：
	反码表示规则为：如果是正数，则表示方法和原码是一样的，如果是负数，则保留符号位1，然后将这个数字的原码按照每位进行取反，则可以得到这歌数的反码表示形式
	数字6在计算中反码就是它的原码：0000 0110
